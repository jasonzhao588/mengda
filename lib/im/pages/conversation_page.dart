...
import 'package:mengda/im/util/combine_message_util.dart';
import 'package:flutter/material.dart';
import 'package:mengda/pages/chats/chat_info.dart';
...
class ConversationPage extends StatefulWidget {
  final Map arguments;
  final People user;
  ConversationPage({Key key, this.arguments, this.user}) : super(key: key);
...
class _ConversationPageState extends State<ConversationPage>
  ...
  void initState() {
    ...
    targetName = arguments["targetNickName"];
    ...
    titleContent = '${widget.user == null ? '--' : widget.user.nickName}';
    ...
    if (conversationType == RCConversationType.Private) {
      ...
      if (_friendInfo != null) {
        setState(() {
          ...
          titleContent = _friendInfo.nickName;
        });
      }
    } else {
      ...
      if (_groupInfo != null) {
        setState(() {
          titleContent =
              '${_groupInfo == null ? '[群聊]' : _groupInfo.groupName}(${_groupInfo.members.length})';
        });
      }
    }
  }
  ...
  void _toChatInfo() {
    Map arg = {"chatType": conversationType, "targetId": targetId};
    // Navigator.pushNamed(context, "/chat_debug", arguments: arg);
    Get.to(
      ChatInfo(),
      arguments: arg,
    ).then((value) => setItemInfo());
  }
