...
class MessageItemFactory extends StatelessWidget {
  ...
  // 被引用的消息 UI
  referenceWidget(ReferenceMessage msg) {
  ...
  Widget messageItem(BuildContext context) {
    if (message.content is TextMessage) {
      ...
      return Padding(
        ...
        child: Text("位置消息 " + message.objectName),
      );
    } else {
      return Container(
        color: Colors.yellow,
  ...
  Color _getMessageWidgetBGColor(int messageDirection) {
    Color color = AppColors.bubbleSendColor;
    if (messageDirection == RCMessageDirection.Receive) {
      color = AppColors.bubbleReciveColor;
    }
    return color;
  }
